...
%% Task 1: Clean Data directory
% % fprintf('Task 1: Cleaning Data directory...\n');
% % dataDir = fullfile('Data');
% % subdirs = dir(dataDir);
% % subdirs = subdirs([subdirs.isdir]); % Keep only directories
% % subdirs = subdirs(~ismember({subdirs.name}, {'.', '..'})); % Remove . and ..
% % pause(5) % Regret minimization
% % for i = 1:length(subdirs)
% %     subdirPath = fullfile(dataDir, subdirs(i).name);
% %     fprintf('Deleting directory: %s\n', subdirPath);
% %     rmdir(subdirPath, 's'); % 's' flag removes directory and all contents
% % end
% % 
% % fprintf('Task 1 completed: All subdirectories removed.\n\n');
% 
% 
...
% % problems = {@BT1, @BT2, @BT3, @BT4, @BT5, @BT6, @BT7, @BT8, @BT9, ...
...
% %     @MinusDTLZ1,@MinusDTLZ2,@MinusDTLZ3,@MinusDTLZ4,@MinusDTLZ5,@MinusDTLZ6,@MinusDTLZ7};
% % 
...
% problems = {@DTLZ1, @DTLZ2, @IDTLZ1, @IDTLZ2, @WFG1, @WFG2, @MaF8, @MaF9};

% ph = problems{1};
% Problem = ph('M', 5)
% [Z,N] = UniformPoint(130,Problem.M);
% Z
% N
% return
